...
- Fifteen notebook files are declared public derivatives. The initial nine replace personal local directories with generic paths. Widget state and rich HTML/widget display payloads are removed. Code and Markdown sources are unchanged except for those original path substitutions; execution counts, plain-text training logs and plain-text evaluation records remain. The six additional recovered exports require no source-path substitutions and preserve all cell sources exactly.
...
`source_manifest.json` retains each supplied input's original `bytes` and `sha256`. A changed public copy additionally has `published_bytes` and `published_sha256`. The artifact auditor checks publication hashes where present and original hashes otherwise; it never treats a public derivative as byte-identical to the original. [The initial transformation report](../reports/publication_sanitization.json) lists the first eleven derivatives; [the recovery source index](../reports/notebook_recovery_sources.json) covers the six additional notebooks.

Original-source audits and superseded `v1` reports describe the files as received before the initial publication pass. Their original hashes are historical identifiers. The earlier `artifact_audit_v2.json` and paired-evaluation reconciliation were regenerated from those declared public derivatives; the recovered 12/20 and 15/20 scores and five-question overlap remain the same. Newly generated reviewer checks cover all 24 bundled source files and the additional recovered records. No model accuracy was created or changed by either publication pass.
