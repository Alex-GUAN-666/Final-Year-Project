...
完整 187 题的 prompt 在本地用固定官方 tokenizer 做了长度检查，最长 252 tokens，符合默认总长度 2,048 / 最大生成 1,024 的预算。云端工作流重建并核对固定数据文件；没有重新下载官方 tokenizer 执行该长度检查。

本地环境未提供 CUDA GPU 和 Docker；真实 Docker 验证已在 GitHub 云端完成。完整 4B 训练与推理、目标 GPU 显存需求、8-bit optimizer 和可选 4-bit 分支仍需在 GPU 环境验证。
...
这一轮交付的是已公开的项目、方法说明、可运行核验入口和通过的云端验证证据。旧权重缺失使历史 checkpoint 不能直接复测；新训练能使用已恢复的数据重新进行，最终提升多少应以真实新结果为准。原始论文包含学号、签名等信息，全文未放入公开仓库；两位作者署名和原 notebook 的上游声明已保留。
